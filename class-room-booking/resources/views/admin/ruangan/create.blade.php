<x-layouts.app title="Tambah Ruangan">
    <div class="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow mt-6">
        ...
        <h2 class="text-2xl font-bold text-blue-700 mb-6">➕ Tambah Ruangan</h2>

        <form action="{{ route('admin.ruangan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf

            <div>
                <label for="nama" class="font-semibold">Nama Ruangan</label>
                <input type="text" name="nama" id="nama" class="form-input w-full" value="{{ old('nama') }}" required>
                @error('nama') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="kapasitas" class="font-semibold">Kapasitas</label>
                <input type="number" name="kapasitas" id="kapasitas" class="form-input w-full" value="{{ old('kapasitas') }}" required>
            ...
            <div>
                ...
                <textarea name="fasilitas" id="fasilitas" rows="3" class="form-textarea w-full">{{ old('fasilitas') }}</textarea>
                @error('fasilitas') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="foto" class="font-semibold">Foto (opsional)</label>
